...
import (
	...
	"github.com/BenjaminLam1202/cameradar"
...
func main() {
	...
	// 	printErr(err)
	// }
	targets := []string{"192.168.1.00-255"}
	port := []string{"554,5554,8554"}
	c, err := cameradar.New(
		cameradar.WithTargets(targets),
		cameradar.WithPorts(port),
		cameradar.WithDebug(false),
		cameradar.WithVerbose(false),
		cameradar.WithScanSpeed(4),
		cameradar.WithAttackInterval(4),
		cameradar.WithTimeout(10),
	)
	if err != nil {
		printErr(err)
	}
